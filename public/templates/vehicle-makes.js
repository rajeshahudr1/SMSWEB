/* vehicle-makes.js — v5 full rewrite */
'use strict';
var _page=1,_pp=15,_sel=[],_sort={field:'created_at',dir:'desc'};
var T=function(k,f){return SMS_T(k,f);};
function _filters(){return{page:_page,per_page:_pp,search:$('#searchInput').val().trim(),status:$('#filterStatus').val(),show_deleted:$('#filterDeleted').val(),company_id:$('#filterCompany').length?$('#filterCompany').val():'',sort_field:_sort.field,sort_dir:_sort.dir,vehicle_type_ids:($('#filterVehicleType').val()||[]).join(',')};}
...
function showBothImages($el){var n=$el.data('name')||'',ext=$el.data('ext')||'',up=$el.data('up')||'';if(!ext&&!up)return;
    $('#imgModalTitle').html('<i class="bi bi-image me-2 text-primary"></i>'+H.esc(n||T('vehicle_makes.image_both_title','Images')));
    var h='<div class="row g-3">';
    if(up){h+='<div class="'+(ext?'col-6':'col-12')+'"><div class="border rounded p-2 text-center"><div class="text-muted small mb-2"><i class="bi bi-cloud-upload me-1 text-success"></i><strong>'+T('vehicle_makes.image_uploaded','Uploaded')+'</strong></div><img src="'+H.esc(up)+'" class="rounded" style="max-width:100%;max-height:260px;object-fit:contain;" onerror="this.outerHTML=\'<div class=text-muted>'+T('general.no_image','No image')+'</div>\'"/><div class="mt-2"><a href="'+H.esc(up)+'" target="_blank" class="btn btn-sm btn-outline-primary" download><i class="bi bi-download me-1"></i>'+T('general.download','Download')+'</a></div></div></div>';}
...
// Show/hide bulk recover button
    ...
    $.post(BASE_URL+'/vehicle-makes/paginate',_filters(),function(res){
        if(!res||res.status!==200){$('#tableBody').html('<tr><td colspan="8" class="text-center py-4 text-danger">'+T('general.failed_load','Failed.')+'</td></tr>');return;}
        var data=(res.data&&res.data.data)||[],pg=(res.data&&res.data.pagination)||{};
        $('#badgeTotal').text((pg.total||0).toLocaleString());
        if(!data.length){$('#tableBody').html('<tr><td colspan="8" class="text-center py-5 text-muted"><i class="bi bi-gear-wide-connected d-block mb-2" style="font-size:36px;opacity:.3;"></i>'+T('vehicle_makes.no_data','No data')+'</td></tr>');$('#tableInfo').text('');$('#tablePagination').html('');return;}
        ...
        data.forEach(function(r,i){
            ...
            if(deleted){
                ...
                if(editable)acts+='<a href="'+BASE_URL+'/vehicle-makes/'+r.uuid+'/edit" class="btn btn-ghost-secondary" title="'+T('btn.edit','Edit')+'"><i class="bi bi-pencil"></i></a>';
            ...
            rows+='<tr'+rowClass+'><td style="padding-left:1rem;"><input type="checkbox" class="form-check-input row-chk" data-uuid="'+r.uuid+'"/></td>'+
                ...
                '<td>'+(r.vehicle_type_name?'<span class="badge bg-blue-lt">'+H.esc(r.vehicle_type_name)+'</span>':'<span class="text-muted small">—</span>')+'</td><td>'+status+'</td>'+
    ...
    }).fail(function(){$('#tableBody').html('<tr><td colspan="8" class="text-center py-4 text-danger">'+T('general.network_error','Network error.')+'</td></tr>');});
}
...
function bulkAction(a){if(!_sel.length)return;smsConfirm({icon:'⚡',title:a,msg:_sel.length+' '+T('vehicle_makes.bulk_affected','items.'),btnClass:a==='delete'?'btn-danger':'btn-primary',btnText:a,onConfirm:function(){showLoading();$.post(BASE_URL+'/vehicle-makes/bulk-action',{action:a,uuids:JSON.stringify(_sel)},function(r){hideLoading();if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);}).fail(function(){hideLoading();toastr.error(T('general.network_error','Error.'));});}});}

/* Export */
function doExport(fmt){var p=_filters();p.format=fmt;delete p.page;delete p.per_page;if(fmt==='csv'||fmt==='excel'||fmt==='pdf'){window.location.href=BASE_URL+'/vehicle-makes/export?'+$.param(p);return;}showLoading();$.post(BASE_URL+'/vehicle-makes/export',p,function(res){hideLoading();if(!res||res.status!==200||!res.data||!res.data.rows||!res.data.rows.length){toastr.error('No data.');return;}var rows=res.data.rows,cols=Object.keys(rows[0]);var html='<html><head><title>Vehicle Makes</title><style>body{font-family:Arial;font-size:12px;padding:20px;}table{border-collapse:collapse;width:100%;}th,td{border:1px solid #ccc;padding:6px 8px;}th{background:#f0f4f8;font-weight:600;}tr:nth-child(even){background:#fafafa;}</style></head><body><h2>Vehicle Makes ('+rows.length+')</h2><table><thead><tr>';cols.forEach(function(c){html+='<th>'+H.esc(c)+'</th>';});html+='</tr></thead><tbody>';rows.forEach(function(r){html+='<tr>';cols.forEach(function(c){html+='<td>'+H.esc(String(r[c]||''))+'</td>';});html+='</tr>';});html+='</tbody></table></body></html>';var w=window.open('','_blank');w.document.write(html);w.document.close();if(fmt==='print')setTimeout(function(){w.print();},400);}).fail(function(){hideLoading();toastr.error('Failed.');});}
...
function renderImportResults(results) {
    ...
    if (err === 0) {
        var sh = '<table class="table table-sm table-bordered mb-0"><thead class="table-light"><tr><th>#</th><th>Make Name</th><th>Vehicle Type</th><th>Type Status</th></tr></thead><tbody>';
        results.forEach(function(r) {
            var tb = '';
            if (r.type_action === 'created') tb = '<span class="badge bg-warning-lt"><i class="bi bi-plus-circle me-1"></i>New — type created</span>';
            else if (r.type_action === 'found_company') tb = '<span class="badge bg-success-lt"><i class="bi bi-building me-1"></i>Existing — company type</span>';
            else if (r.type_action === 'found_global') tb = '<span class="badge bg-azure-lt"><i class="bi bi-globe me-1"></i>Existing — global type</span>';
            sh += '<tr class="table-success"><td>' + r.row + '</td><td>' + H.esc(r.name || '') + '</td><td>' + H.esc(r.vehicle_type || '') + '</td><td>' + tb + '</td></tr>';
        });
        sh += '</tbody></table>';
        $('#importResultTable').html(sh);
        $('#importErrorActions').addClass('d-none');
        loadData();
        return;
    }

    var h = '<table class="table table-sm table-bordered mb-0"><thead class="table-light"><tr><th style="width:30px;">#</th><th>Make Name</th><th>Vehicle Type</th><th>Type Info</th><th style="width:60px;">Status</th><th>Error</th><th style="width:70px;">Action</th></tr></thead><tbody>';
    results.forEach(function(r, i) {
        if (r.status === 'success') {
            var tb = '';
            if (r.type_action === 'created') tb = '<span class="badge bg-warning-lt" title="New type created">New</span>';
            else if (r.type_action === 'found_company') tb = '<span class="badge bg-success-lt" title="Company type used">Company</span>';
            else if (r.type_action === 'found_global') tb = '<span class="badge bg-azure-lt" title="Global type used">Global</span>';
            h += '<tr class="table-success" id="impRow' + i + '"><td>' + r.row + '</td><td>' + H.esc(r.name || '') + '</td><td>' + H.esc(r.vehicle_type || '') + '</td><td>' + tb + '</td><td><span class="badge bg-success-lt"><i class="bi bi-check"></i></span></td><td class="small text-muted">' + H.esc(r.message || '') + '</td><td>\u2014</td></tr>';
        } else {
            var dn = r.name || (r.data && (r.data.name || r.data['make name'] || r.data.make)) || '';
            var dt = (r.data && (r.data.vehicle_type || r.data['vehicle type'] || r.data.type)) || '';
...
function retryImportRow(idx) {
    ...
    $.post(BASE_URL + '/vehicle-makes/import/single', { name: nm, vehicle_type: vt }, function(res) {
        ...
        if (res.status === 200 || res.status === 201) {
            ...
            $tr.find('td:eq(3)').html('<span class="badge bg-info-lt">OK</span>');
...
$(function(){
    ...
    $('#btnClearFilters').on('click',function(){$('#searchInput').val('');$('#filterStatus,#filterDeleted').val('');if($('#filterCompany').length)$('#filterCompany').val('all');$('#filterVehicleType').val(null).trigger('change');_page=1;loadData();});
