...
function _filters(){var p={page:_page,per_page:_pp,search:$('#searchInput').val().trim(),status:$('#filterStatus').val(),show_deleted:$('#filterDeleted').val(),sort_field:_sort.field,sort_dir:_sort.dir};var fk=$('#filterFK').val()||''; if(fk)p.country_id=fk;return p;}
...
function loadData(){
    ...
    $.post(BASE_URL+'/states/paginate',_filters(),function(res){
        ...
        if(!data.length){$('#tableBody').html('<tr><td colspan="6" class="text-center py-5 text-muted"><i class="bi bi-geo-alt d-block mb-2" style="font-size:36px;opacity:.3;"></i>No states found</td></tr>');$('#tableInfo').text('');$('#tablePagination').html('');return;}
        ...
        data.forEach(function(r,i){
            ...
            rows+='<td class="d-none d-md-table-cell">'+H.esc(r.country_name||'')+'</td>';
            rows+='<td>'+st+'</td>';
...
function viewRec(uuid){var $b=$('#viewBody');$b.html('<div class="text-center py-5"><div class="spinner-border text-primary"></div></div>');bootstrap.Modal.getOrCreateInstance($('#modalView')[0]).show();
    $.get(BASE_URL+'/states/'+uuid+'/view-data',function(res){
        ...
        h+='<div class="mb-2"><span class="text-muted small">Country:</span> '+H.esc(r.country_name||'—')+'</div>';
...
function showUsage(uuid, name) {
    ...
    $.get(BASE_URL + '/states/' + uuid + '/usage', function(res) {
...
function toggleRec(u){$.post(BASE_URL+'/states/'+u+'/toggle-status',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}
function delRec(u,n){smsConfirm({title:T('btn.delete','Delete'),msg:T('btn.delete','Delete')+' <strong>'+H.esc(n)+'</strong>?',btnClass:'btn-danger',btnText:T('btn.delete','Delete'),onConfirm:function(){$.post(BASE_URL+'/states/'+u+'/delete',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}});}
function recoverRec(u,n){smsConfirm({title:T('bulk.recover','Recover'),msg:T('bulk.recover','Recover')+' <strong>'+H.esc(n)+'</strong>?',btnClass:'btn-success',btnText:T('bulk.recover','Recover'),onConfirm:function(){$.post(BASE_URL+'/states/'+u+'/recover',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}});}
...
function bulkAction(a){if(!_sel.length)return;var _t=a==='delete'?T('btn.delete','Delete'):a==='recover'?T('bulk.recover','Recover'):a;smsConfirm({title:_t,msg:_sel.length+' '+T('bulk.items_will_be','items will be')+' '+_t.toLowerCase()+'.',btnClass:a==='delete'?'btn-danger':'btn-primary',btnText:_t,onConfirm:function(){$.ajax({url:BASE_URL+'/states/bulk-action',type:'POST',contentType:'application/json',data:JSON.stringify({action:a,uuids:_sel}),success:function(r){if(r.status===200){toastr.success(r.message);$('#selectAll').prop('checked',false);loadData();}else toastr.error(r.message);}});}});}

function doExport(fmt){var p=_filters();p.format=fmt;delete p.page;delete p.per_page;
    if(fmt==='csv'||fmt==='excel'||fmt==='pdf'){window.location.href=BASE_URL+'/states/export?'+$.param(p);return;}
    showLoading();$.post(BASE_URL+'/states/export',p,function(res){hideLoading();if(!res||res.status!==200||!res.data||!res.data.rows||!res.data.rows.length){toastr.error(T('general.no_data','No data.'));return;}
        var rows=res.data.rows,cols=Object.keys(rows[0]);var html='<html><head><title>States</title><style>body{font-family:Arial;font-size:11px;padding:20px;}table{border-collapse:collapse;width:100%;}th,td{border:1px solid #ccc;padding:4px 6px;}th{background:#f0f4f8;font-weight:600;}</style></head><body><h2>States ('+rows.length+')</h2><table><tr>';
...
function renderImportResults(results) {
    ...
    results.forEach(function(r, i) {
        ...
        var dc = d.country || d['country'] || d.country_name || '';
        h += '<tr class="table-danger" id="impRow' + i + '">' +
            ...
            '<td><input type="text" class="form-control form-control-sm imp-country" value="' + H.esc(dc) + '" placeholder="Country"/></td>' +
...
function retryImportRow(idx){
    ...
    var ct=$tr.find('.imp-country').val().trim();
    if(!nm){toastr.error(T('msg.name_required','Name required.'));return;}
    var $b=$tr.find('button');btnLoading($b);
    $.post(BASE_URL+'/states/import/single',{name:nm,country:ct},function(res){
        btnReset($b);
...
$(function(){
    ...
    // Country filter — Select2 autocomplete
    $('#filterFK').select2({ placeholder: 'All Countries', allowClear: true, width: '100%',
        ajax: { url: BASE_URL + '/countries/autocomplete', dataType: 'json', delay: 300,
    ...
    $('#frmImport').on('submit',function(e){e.preventDefault();var fd=new FormData(this),$b=$('#btnImport');btnLoading($b);
        ...
        $.ajax({url:BASE_URL+'/states/import',type:'POST',data:fd,processData:false,contentType:false,
