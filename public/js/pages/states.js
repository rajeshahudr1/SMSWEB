...
function delRec(u,n){smsConfirm({title:'Delete',msg:'Delete <strong>'+H.esc(n)+'</strong>?',btnClass:'btn-danger',btnText:'Delete',onConfirm:function(){$.post(BASE_URL+'/states/'+u+'/delete',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}});}
function recoverRec(u,n){smsConfirm({title:'Recover',msg:'Recover <strong>'+H.esc(n)+'</strong>?',btnClass:'btn-success',btnText:'Recover',onConfirm:function(){$.post(BASE_URL+'/states/'+u+'/recover',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}});}
...
function doExport(fmt){var p=_filters();p.format=fmt;delete p.page;delete p.per_page;
    ...
    showLoading();$.post(BASE_URL+'/states/export',p,function(res){hideLoading();if(!res||res.status!==200||!res.data||!res.data.rows||!res.data.rows.length){toastr.error('No data.');return;}
...
function retryRow(idx){
    ...
    $.post(BASE_URL+'/states/import/single',{name:nm},function(res){
