...
function _filters(){var p={page:_page,per_page:_pp,search:$('#searchInput').val().trim(),status:$('#filterStatus').val(),show_deleted:$('#filterDeleted').val(),sort_field:_sort.field,sort_dir:_sort.dir};var fk=$('#filterFK').val()||''; if(fk)p.state_id=fk;return p;}
...
function loadData(){
    ...
    $.post(BASE_URL+'/cities/paginate',_filters(),function(res){
        if(!res||res.status!==200){$('#tableBody').html('<tr><td colspan="6" class="text-center py-4 text-danger">'+T('msg.failed','Failed.')+'</td></tr>');return;}
        var data=(res.data&&res.data.data)||[],pg=(res.data&&res.data.pagination)||{};
        $('#badgeTotal').text((pg.total||0).toLocaleString());
        if(!data.length){$('#tableBody').html('<tr><td colspan="6" class="text-center py-5 text-muted"><i class="bi bi-geo-alt d-block mb-2" style="font-size:36px;opacity:.3;"></i>No cities found</td></tr>');$('#tableInfo').text('');$('#tablePagination').html('');return;}
        ...
        data.forEach(function(r,i){
            ...
            if(del){
                ...
                acts+='<li><a class="dropdown-item" href="'+BASE_URL+'/cities/'+r.uuid+'/edit"><i class="bi bi-pencil me-2 text-secondary"></i>Edit</a></li>';
            ...
            rows+='<td class="d-none d-md-table-cell">'+H.esc(r.state_name||'')+'</td>';
            rows+='<td>'+st+'</td>';
...
function viewRec(uuid){var $b=$('#viewBody');$b.html('<div class="text-center py-5"><div class="spinner-border text-primary"></div></div>');bootstrap.Modal.getOrCreateInstance($('#modalView')[0]).show();
    $.get(BASE_URL+'/cities/'+uuid+'/view-data',function(res){
        ...
        h+='<div class="mb-2"><span class="text-muted small">State:</span> '+H.esc(r.state_name||'—')+'</div>';
...
function showUsage(uuid, name) {
    ...
    $.get(BASE_URL + '/cities/' + uuid + '/usage', function(res) {
...
function toggleRec(u){$.post(BASE_URL+'/cities/'+u+'/toggle-status',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}
function delRec(u,n){smsConfirm({title:T('btn.delete','Delete'),msg:T('btn.delete','Delete')+' <strong>'+H.esc(n)+'</strong>?',btnClass:'btn-danger',btnText:T('btn.delete','Delete'),onConfirm:function(){$.post(BASE_URL+'/cities/'+u+'/delete',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}});}
function recoverRec(u,n){smsConfirm({title:T('bulk.recover','Recover'),msg:T('bulk.recover','Recover')+' <strong>'+H.esc(n)+'</strong>?',btnClass:'btn-success',btnText:T('bulk.recover','Recover'),onConfirm:function(){$.post(BASE_URL+'/cities/'+u+'/recover',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}});}
...
function bulkAction(a){if(!_sel.length)return;var _t=a==='delete'?T('btn.delete','Delete'):a==='recover'?T('bulk.recover','Recover'):a;smsConfirm({title:_t,msg:_sel.length+' '+T('bulk.items_will_be','items will be')+' '+_t.toLowerCase()+'.',btnClass:a==='delete'?'btn-danger':'btn-primary',btnText:_t,onConfirm:function(){$.ajax({url:BASE_URL+'/cities/bulk-action',type:'POST',contentType:'application/json',data:JSON.stringify({action:a,uuids:_sel}),success:function(r){if(r.status===200){toastr.success(r.message);$('#selectAll').prop('checked',false);loadData();}else toastr.error(r.message);}});}});}

function doExport(fmt){var p=_filters();p.format=fmt;delete p.page;delete p.per_page;
    if(fmt==='csv'||fmt==='excel'||fmt==='pdf'){window.location.href=BASE_URL+'/cities/export?'+$.param(p);return;}
    showLoading();$.post(BASE_URL+'/cities/export',p,function(res){hideLoading();if(!res||res.status!==200||!res.data||!res.data.rows||!res.data.rows.length){toastr.error(T('general.no_data','No data.'));return;}
        var rows=res.data.rows,cols=Object.keys(rows[0]);var html='<html><head><title>Cities</title><style>body{font-family:Arial;font-size:11px;padding:20px;}table{border-collapse:collapse;width:100%;}th,td{border:1px solid #ccc;padding:4px 6px;}th{background:#f0f4f8;font-weight:600;}</style></head><body><h2>Cities ('+rows.length+')</h2><table><tr>';
...
function renderImportResults(results) {
    ...
    results.forEach(function(r, i) {
        ...
        var ds = d.state || d['state'] || d.state_name || '';
        h += '<tr class="table-danger" id="impRow' + i + '">' +
            ...
            '<td><input type="text" class="form-control form-control-sm imp-state" value="' + H.esc(ds) + '" placeholder="State"/></td>' +
...
function retryImportRow(idx){
    ...
    var st=$tr.find('.imp-state').val().trim();
    if(!nm){toastr.error(T('msg.name_required','Name required.'));return;}
    var $b=$tr.find('button');btnLoading($b);
    $.post(BASE_URL+'/cities/import/single',{name:nm,state:st},function(res){
        btnReset($b);
...
$(function(){
    ...
    // State filter — Select2 autocomplete
    $('#filterFK').select2({ placeholder: 'All States', allowClear: true, width: '100%',
        ajax: { url: BASE_URL + '/states/autocomplete', dataType: 'json', delay: 300,
    ...
    $('#btnClearFilters').on('click',function(){$('#searchInput').val('');$('#filterStatus,#filterDeleted').val('');$('#filterFK').val(null).trigger('change');_page=1;loadData();});
    ...
    $('#frmImport').on('submit',function(e){e.preventDefault();var fd=new FormData(this),$b=$('#btnImport');btnLoading($b);
        ...
        $.ajax({url:BASE_URL+'/cities/import',type:'POST',data:fd,processData:false,contentType:false,
