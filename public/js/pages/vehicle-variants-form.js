...
$(function() {
    ...
    $('#selVehicleYear').select2({ placeholder: 'Select Year...', allowClear: true, width: '100%',
    ...
    $('#selVehicleType').select2({ placeholder: 'Select Type...', allowClear: true, width: '100%',
        ajax: { url: BASE_URL + '/vehicle-types/autocomplete', dataType: 'json', delay: 300,
            data: function(params) { return { search: params.term || '', limit: '' }; },
            processResults: function(res) { return { results: (res.data||[]).map(function(r){ return {id:r.id,text:r.name}; }) }; }, cache: true
        }, minimumInputLength: 0
    });
    $('#selVehicleMake').select2({ placeholder: 'Select Make...', allowClear: true, width: '100%',
        ajax: { url: BASE_URL + '/vehicle-makes/autocomplete', dataType: 'json', delay: 300,
            data: function(params) { var d = { search: params.term || '', limit: '' }; d.vehicle_type_id = $('#selVehicleType').val() || ''; return d; },
            processResults: function(res) { return { results: (res.data||[]).map(function(r){ return {id:r.id,text:r.name}; }) }; }, cache: true
        }, minimumInputLength: 0
    });
    $('#selVehicleModel').select2({ placeholder: 'Select Model...', allowClear: true, width: '100%',
        ajax: { url: BASE_URL + '/vehicle-models/autocomplete', dataType: 'json', delay: 300,
            data: function(params) { var d = { search: params.term || '', limit: '' }; d.vehicle_type_id = $('#selVehicleType').val() || ''; d.vehicle_make_id = $('#selVehicleMake').val() || ''; return d; },
            processResults: function(res) { return { results: (res.data||[]).map(function(r){ return {id:r.id,text:r.name}; }) }; }, cache: true
        }, minimumInputLength: 0
    });
    $('#selVehicleType').on('change', function() { $('#selVehicleMake').val(null).trigger('change'); $('#selVehicleModel').val(null).trigger('change'); });
    $('#selVehicleMake').on('change', function() { $('#selVehicleModel').val(null).trigger('change'); });
    var _aiConfig = null;
