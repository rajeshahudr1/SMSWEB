...
$(function() {
    ...
    $('#frmPartType').on('submit', function(e) {
        ...
        $.ajax({ url: $(this).attr('action'), type: 'POST', data: fd, processData: false, contentType: false,
            success: function(r) { btnReset($btn); if (r.status === 200 || r.status === 201) { toastr.success(r.message || T('msg.settings_saved','Saved.')); setTimeout(function() { window.location = '/vehicle-variants'; }, 800); } else toastr.error(r.message || T('general.error','Error.')); },
