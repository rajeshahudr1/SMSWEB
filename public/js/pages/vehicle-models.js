/* vehicle-models.js — list page (v2) */
'use strict';
...
function loadData(){
    $('#tableBody').html('<tr><td colspan="12" class="text-center py-5 text-muted"><div class="spinner-border spinner-border-sm text-primary me-2"></div>Loading...</td></tr>');
    ...
    $.post(BASE_URL+'/vehicle-models/paginate',_filters(),function(res){
        if(!res||res.status!==200){$('#tableBody').html('<tr><td colspan="12" class="text-center py-4 text-danger">Failed.</td></tr>');return;}
        var data=(res.data&&res.data.data)||[],pg=(res.data&&res.data.pagination)||{};
        $('#badgeTotal').text((pg.total||0).toLocaleString());
        if(!data.length){$('#tableBody').html('<tr><td colspan="12" class="text-center py-5 text-muted"><i class="bi bi-car-front-fill d-block mb-2" style="font-size:36px;opacity:.3;"></i>No vehicle models found</td></tr>');$('#tableInfo').text('');$('#tablePagination').html('');return;}
        ...
        data.forEach(function(r,i){
            ...
            var yearBadge = r.vehicle_year_name ? '<span class="badge bg-cyan-lt">' + H.esc(r.vehicle_year_name) + '</span>' : '<span class="text-muted">—</span>';
            var typeBadge = r.vehicle_type_name ? '<span class="badge bg-blue-lt">' + H.esc(r.vehicle_type_name) + '</span>' : '<span class="text-muted">—</span>';
            var makeBadge = r.vehicle_make_name ? '<span class="badge bg-purple-lt">' + H.esc(r.vehicle_make_name) + '</span>' : '<span class="text-muted">—</span>';
            ...
            acts+='<button class="btn btn-ghost-primary btn-sm" onclick="viewRec(\''+r.uuid+'\')"><i class="bi bi-eye"></i></button>';
            if(deleted){acts+='<button class="btn btn-ghost-success btn-sm" onclick="recoverRec(\''+r.uuid+'\',\''+H.esc(r.name||'')+'\')"><i class="bi bi-arrow-counterclockwise"></i></button>';}
            else{
                if(ed)acts+='<a href="'+BASE_URL+'/vehicle-models/'+r.uuid+'/edit" class="btn btn-ghost-secondary btn-sm"><i class="bi bi-pencil"></i></a>';
                if(ed)acts+='<button class="btn btn-ghost-'+(parseInt(r.status)?'warning':'success')+' btn-sm" onclick="toggleRec(\''+r.uuid+'\')"><i class="bi bi-toggle-'+(parseInt(r.status)?'on':'off')+'"></i></button>';
                if(dl)acts+='<button class="btn btn-ghost-danger btn-sm" onclick="delRec(\''+r.uuid+'\',\''+H.esc(r.name||'')+'\')"><i class="bi bi-trash3"></i></button>';
            }
            acts+='</div>';
            ...
            rows+='<tr'+rc+'><td><input type="checkbox" class="form-check-input row-chk" data-uuid="'+r.uuid+'"/></td>'+
                ...
                '<td>'+vmImg(r)+'</td>'+
                '<td><span class="fw-medium">'+H.esc(r.name||'')+'</span>'+gb+'</td>'+
                '<td>'+typeBadge+'</td>'+
                '<td>'+yearBadge+'</td>'+
                '<td>'+makeBadge+'</td>'+
                '<td>'+prodStart+'</td>'+
    ...
    }).fail(function(){$('#tableBody').html('<tr><td colspan="12" class="text-center py-4 text-danger">Network error.</td></tr>');});
}
...
function toggleRec(u){smsConfirm({title:'Confirm',text:'Toggle status?',onConfirm:function(){$.post(BASE_URL+'/vehicle-models/'+u+'/toggle-status',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}});}
function delRec(u,n){smsConfirm({title:'Delete',text:'Delete <strong>'+n+'</strong>?',btnClass:'btn-danger',onConfirm:function(){$.post(BASE_URL+'/vehicle-models/'+u+'/delete',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}});}
function recoverRec(u,n){smsConfirm({title:'Recover',text:'Recover <strong>'+n+'</strong>?',btnClass:'btn-success',onConfirm:function(){$.post(BASE_URL+'/vehicle-models/'+u+'/recover',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}});}
...
function doExport(fmt){
    ...
    $.post(BASE_URL+'/vehicle-models/export',p,function(res){
        ...
        var html='<html><head><title>Vehicle Models</title><style>body{font-family:Arial;font-size:11px;padding:20px;}table{border-collapse:collapse;width:100%;}th,td{border:1px solid #ccc;padding:4px 6px;}th{background:#f0f4f8;font-weight:600;}</style></head><body><h2>Vehicle Models ('+rows.length+')</h2><table><tr>';
...
function renderImportResults(results) {
    ...
    $('#importSummary').html('<div class="d-flex gap-2 flex-wrap"><span class="badge bg-success-lt px-3 py-2"><i class="bi bi-check-circle me-1"></i>' + ok + ' imported</span><span class="badge bg-danger-lt px-3 py-2"><i class="bi bi-x-circle me-1"></i>' + err + ' errors</span></div>');
    ...
    var h = '<table class="table table-sm table-bordered mb-0" style="font-size:11px;white-space:nowrap;"><thead class="table-light"><tr>' +
        ...
        '<th style="min-width:110px;">Model Name</th>' +
        '<th style="min-width:90px;">Vehicle Type</th>' +
        '<th style="min-width:90px;">Vehicle Make</th>' +
    ...
    results.forEach(function(r, i) {
        if (r.status === 'success') {
            var ta = r.type_action === 'created' ? ' <sup class="text-warning fw-bold">NEW</sup>' : '';
            var ma = r.make_action === 'created' ? ' <sup class="text-warning fw-bold">NEW</sup>' : '';
            var ya = r.year_action === 'created' ? ' <sup class="text-warning fw-bold">NEW</sup>' : '';
            h += '<tr class="table-success" id="impRow' + i + '">' +
                '<td>' + r.row + '</td>' +
                '<td>' + H.esc(r.name||'') + '</td>' +
                '<td>' + H.esc(r.vehicle_type||'') + ta + '</td>' +
                '<td>' + H.esc(r.vehicle_make||'') + ma + '</td>' +
                '<td>' + H.esc(r.vehicle_year||'') + ya + '</td>' +
                '<td>' + H.esc(r.start_year||'') + '</td>' +
                '<td>' + H.esc(r.month_initial||'') + '</td>' +
                '<td>' + H.esc(r.end_year||'') + '</td>' +
                '<td>' + H.esc(r.month_final||'') + '</td>' +
                '<td class="text-truncate" style="max-width:90px;" title="' + H.esc(r.image_url||'') + '">' + H.esc(r.image_url||'') + '</td>' +
                '<td><i class="bi bi-check-circle text-success"></i></td>' +
                '<td class="small text-muted">' + H.esc(r.message||'') + '</td>' +
                '<td>\u2014</td></tr>';
        } else {
            var d = r.data || {};
            var dn = r.name || d.name || d['model name'] || d['model_name'] || d.model || '';
...
function retryImportRow(idx) {
    ...
    if (!nm) { toastr.error('Model name is required.'); return; }
    ...
    $.post(BASE_URL + '/vehicle-models/import/single', {
        name: nm, vehicle_type: vt, vehicle_make: vmk, vehicle_year: vy,
        month_initial: mi, month_final: mf, start_year: sy, end_year: ey, image_url: iu
    }, function(res) {
        btnReset($b);
        if (res.status === 200 || res.status === 201) {
            $tr.removeClass('table-danger').addClass('table-success');
            $tr.find('input').prop('disabled', true).addClass('bg-light');
            $tr.find('td:eq(10)').html('<i class="bi bi-check-circle text-success"></i>');
            $tr.find('td:eq(11)').text(res.message || 'OK').removeClass('text-danger').addClass('text-muted');
...
$(function(){
    ...
    $('#frmImport').on('submit', function(e) { e.preventDefault(); var fd = new FormData(this), $b = $('#btnImport'); btnLoading($b);
        $.ajax({ url: BASE_URL + '/vehicle-models/import', type: 'POST', data: fd, processData: false, contentType: false,
            success: function(r) { btnReset($b); if (r.status === 200 && r.data && r.data.results) { $('#importStep1').hide(); $('#importStep2').removeClass('d-none'); renderImportResults(r.data.results); } else toastr.error(r.message || 'Failed.'); },
