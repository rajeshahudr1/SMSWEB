...
function loadData(){
    ...
    $.post(BASE_URL+'/vehicle-models/paginate',_filters(),function(res){
        ...
        if(!data.length){$('#tableBody').html('<tr><td colspan="12" class="text-center py-5 text-muted"><i class="bi bi-car-front-fill d-block mb-2" style="font-size:36px;opacity:.3;"></i>'+T('msg.no_vehicle_models','No vehicle models found')+'</td></tr>');$('#tableInfo').text('');$('#tablePagination').html('');return;}
        ...
        data.forEach(function(r,i){
            ...
            if(deleted){acts+='<button class="btn btn-ghost-success btn-sm" onclick="recoverRec(\''+r.uuid+'\',\''+H.esc(r.name||'')+'\')"><i class="bi bi-arrow-counterclockwise"></i></button>';}
                ...
                acts+='<button class="btn btn-ghost-info btn-sm" onclick="showUsage(\''+r.uuid+'\',\''+H.esc(r.name||'')+'\')"><i class="bi bi-diagram-3"></i></button>';
...
function showUsage(uuid, name) {
    ...
    $.get(BASE_URL + '/vehicle-models/' + uuid + '/usage', function(res) {
        if (!res || res.status !== 200) { $b.html('<div class="alert alert-danger m-3">' + T('general.failed_load', 'Failed.') + '</div>'); return; }
        smsRenderUsageBody(res.data, 'vehicle-models', uuid, name);
...
function toggleRec(u){smsConfirm({title:T('general.confirm','Confirm'),text:T('msg.toggle_status','Toggle status?'),onConfirm:function(){$.post(BASE_URL+'/vehicle-models/'+u+'/toggle-status',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}});}
function delRec(u,n){smsConfirm({title:T('btn.delete','Delete'),text:T('btn.delete','Delete')+' <strong>'+n+'</strong>?',btnClass:'btn-danger',onConfirm:function(){$.post(BASE_URL+'/vehicle-models/'+u+'/delete',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}});}
function recoverRec(u,n){smsConfirm({title:T('bulk.recover','Recover'),text:T('bulk.recover','Recover')+' <strong>'+n+'</strong>?',btnClass:'btn-success',onConfirm:function(){$.post(BASE_URL+'/vehicle-models/'+u+'/recover',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}});}
...
function doExport(fmt){
    ...
    if(fmt==='csv'||fmt==='excel'||fmt==='pdf'){
        ...
        $.post(BASE_URL+'/vehicle-models/export',checkP,function(res){
            hideLoading();
            ...
            window.location.href=BASE_URL+'/vehicle-models/export?'+$.param(p);
...
function retryImportRow(idx) {
    ...
    if (!nm) { toastr.error(T('msg.name_required','Model name is required.')); return; }
    ...
    $.post(BASE_URL + '/vehicle-models/import/single', {
        ...
        if (res.status === 200 || res.status === 201) {
            ...
            $tr.find('td:eq(10)').html('<span class="text-success small"><i class="bi bi-check-circle me-1"></i>OK</span>');
