/* vehicle-categories.js — v5 full rewrite */
'use strict';
...
function showBothImages($el){var n=$el.data('name')||'',ext=$el.data('ext')||'',up=$el.data('up')||'';if(!ext&&!up)return;
    $('#imgModalTitle').html('<i class="bi bi-image me-2 text-primary"></i>'+H.esc(n||T('vehicle_categories.image_both_title','Images')));
    var h='<div class="row g-3">';
    if(up){h+='<div class="'+(ext?'col-6':'col-12')+'"><div class="border rounded p-2 text-center"><div class="text-muted small mb-2"><i class="bi bi-cloud-upload me-1 text-success"></i><strong>'+T('vehicle_categories.image_uploaded','Uploaded')+'</strong></div><img src="'+H.esc(up)+'" class="rounded" style="max-width:100%;max-height:260px;object-fit:contain;" onerror="this.outerHTML=\'<div class=text-muted>'+T('general.no_image','No image')+'</div>\'"/><div class="mt-2"><a href="'+H.esc(up)+'" target="_blank" class="btn btn-sm btn-outline-primary" download><i class="bi bi-download me-1"></i>'+T('general.download','Download')+'</a></div></div></div>';}
...
// Show/hide bulk recover button
    ...
    $.post(BASE_URL+'/vehicle-categories/paginate',_filters(),function(res){
        ...
        if(!data.length){$('#tableBody').html('<tr><td colspan="7" class="text-center py-5 text-muted"><i class="bi bi-gear-wide-connected d-block mb-2" style="font-size:36px;opacity:.3;"></i>'+T('vehicle_categories.no_data','No data')+'</td></tr>');$('#tableInfo').text('');$('#tablePagination').html('');return;}
        ...
        data.forEach(function(r,i){
            ...
            if(deleted){
                ...
                if(editable)acts+='<li><a class="dropdown-item" href="'+BASE_URL+'/vehicle-categories/'+r.uuid+'/edit"><i class="bi bi-pencil me-2 text-secondary"></i>Edit</a></li>';
...
function viewPT(uuid){var $b=$('#viewBody');$b.html('<div class="text-center py-5"><div class="spinner-border text-primary"></div></div>');bootstrap.Modal.getOrCreateInstance($('#modalView')[0]).show();
    $.get(BASE_URL+'/vehicle-categories/'+uuid+'/view-data',function(res){
...
function bulkAction(a){if(!_sel.length)return;smsConfirm({icon:'⚡',title:a,msg:_sel.length+' '+T('vehicle_categories.bulk_affected','items.'),btnClass:a==='delete'?'btn-danger':'btn-primary',btnText:a,onConfirm:function(){showLoading();$.post(BASE_URL+'/vehicle-categories/bulk-action',{action:a,uuids:JSON.stringify(_sel)},function(r){hideLoading();if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);}).fail(function(){hideLoading();toastr.error(T('general.network_error','Error.'));});}});}

/* Export */
function doExport(fmt){var p=_filters();p.format=fmt;delete p.page;delete p.per_page;if(fmt==='csv'||fmt==='excel'||fmt==='pdf'){window.location.href=BASE_URL+'/vehicle-categories/export?'+$.param(p);return;}showLoading();$.post(BASE_URL+'/vehicle-categories/export',p,function(res){hideLoading();if(!res||res.status!==200||!res.data||!res.data.rows||!res.data.rows.length){toastr.error(T('general.no_data','No data.'));return;}var rows=res.data.rows,cols=Object.keys(rows[0]);var html='<html><head><title>Vehicle Categories</title><style>body{font-family:Arial;font-size:12px;padding:20px;}table{border-collapse:collapse;width:100%;}th,td{border:1px solid #ccc;padding:6px 8px;}th{background:#f0f4f8;font-weight:600;}tr:nth-child(even){background:#fafafa;}</style></head><body><h2>Vehicle Categories ('+rows.length+')</h2><table><thead><tr>';cols.forEach(function(c){html+='<th>'+H.esc(c)+'</th>';});html+='</tr></thead><tbody>';rows.forEach(function(r){html+='<tr>';cols.forEach(function(c){html+='<td>'+H.esc(String(r[c]||''))+'</td>';});html+='</tr>';});html+='</tbody></table></body></html>';var w=window.open('','_blank');w.document.write(html);w.document.close();if(fmt==='print')setTimeout(function(){w.print();},400);}).fail(function(){hideLoading();toastr.error(T('msg.failed','Failed.'));});}
/* ── Import ── */
...
function showUsage(uuid, name) {
    ...
    $.get(BASE_URL + '/vehicle-categories/' + uuid + '/usage', function(res) {
        if (!res || res.status !== 200) { $b.html('<div class="alert alert-danger m-3">' + T('general.failed_load', 'Failed.') + '</div>'); return; }
        smsRenderUsageBody(res.data, 'vehicle-categories', uuid, name);
