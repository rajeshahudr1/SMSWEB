...
// Show/hide bulk recover button
    ...
    $.post(BASE_URL+'/vehicle-makes/paginate',_filters(),function(res){
        ...
        if(!data.length){$('#tableBody').html('<tr><td colspan="7" class="text-center py-5 text-muted"><i class="bi bi-gear-wide-connected d-block mb-2" style="font-size:36px;opacity:.3;"></i>'+T('vehicle_makes.no_data','No data')+'</td></tr>');$('#tableInfo').text('');$('#tablePagination').html('');return;}
...
function retryImportRow(idx) {
    ...
    $.post(BASE_URL + '/vehicle-makes/import/single', { name: newName, image_url: (r.data && r.data['image url']) || '' }, function(res) {
