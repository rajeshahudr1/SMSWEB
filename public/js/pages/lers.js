/* lers.js — LER Codes list page */
'use strict';
var _page=1,_pp=15,_sel=[],_sort={field:'created_at',dir:'desc'};
function _filters(){return{page:_page,per_page:_pp,search:$('#searchInput').val().trim(),status:$('#filterStatus').val(),show_deleted:$('#filterDeleted').val(),company_id:$('#filterCompany').length?$('#filterCompany').val():'',unit_id:$('#filterUnit').val(),is_body_ler:$('#filterBodyLer').val(),sort_field:_sort.field,sort_dir:_sort.dir};}
...
function loadData(){
    ...
    $.post(BASE_URL+'/lers/paginate',_filters(),function(res){
        ...
        if(!data.length){$('#tableBody').html('<tr><td colspan="10" class="text-center py-5 text-muted"><i class="bi bi-recycle d-block mb-2" style="font-size:36px;opacity:.3;"></i>No LER codes found</td></tr>');$('#tableInfo').text('');$('#tablePagination').html('');return;}
        ...
        data.forEach(function(r,i){
            ...
            // Unit display
            var unitDisplay = r.unit_name ? H.esc(r.unit_name) : '<span class="text-muted">-</span>';
            if (r.unit_name && r.unit_symbol) unitDisplay += ' <span class="text-muted small">(' + H.esc(r.unit_symbol) + ')</span>';

            // Default value display
            var defVal = r.default_unit_value != null && r.default_unit_value !== '' ? parseFloat(r.default_unit_value).toFixed(4) : '<span class="text-muted">-</span>';

            // Body LER badge
            var bodyBadge = r.is_body_ler ? '<span class="badge bg-primary-lt">Yes</span>' : '<span class="badge bg-secondary-lt">No</span>';
            ...
            acts+='<li><a class="dropdown-item" href="#" onclick="viewLer(\''+r.uuid+'\');return false;"><i class="bi bi-eye me-2 text-primary"></i>View Details</a></li>';
            if(deleted){
                acts+='<li><a class="dropdown-item" href="#" onclick="recoverLer(\''+r.uuid+'\',\''+H.esc(r.name||'')+'\');return false;"><i class="bi bi-arrow-counterclockwise me-2 text-success"></i>Recover</a></li>';
            }else{
                if(editable)acts+='<li><a class="dropdown-item" href="'+BASE_URL+'/lers/'+r.uuid+'/edit"><i class="bi bi-pencil me-2 text-secondary"></i>Edit</a></li>';
                if(editable)acts+='<li><a class="dropdown-item" href="#" onclick="toggleLer(\''+r.uuid+'\');return false;"><i class="bi bi-toggle-'+(isActive?'off':'on')+' me-2 text-'+(isActive?'warning':'success')+'"></i>'+(isActive?'Deactivate':'Activate')+'</a></li>';
                if(deletable){acts+='<li><hr class="dropdown-divider"></li><li><a class="dropdown-item text-danger" href="#" onclick="delLer(\''+r.uuid+'\',\''+H.esc(r.name||'')+'\');return false;"><i class="bi bi-trash3 me-2"></i>Delete</a></li>';}
            }
            acts+='</ul></div>';
...
function viewLer(uuid){
    ...
    $.get(BASE_URL+'/lers/'+uuid+'/view-data',function(res){
...
function renderViewModal(lr){
    var $b=$('#viewBody');
    var h='<div class="p-4">';
    h+='<div class="text-center mb-4"><h4 class="mb-1">'+H.esc(lr.name||'')+'</h4>';
    if(lr.code) h+='<div class="mb-1"><code class="fs-6">'+H.esc(lr.code)+'</code></div>';
    h+='<div>'+((lr.status===true||lr.status===1||parseInt(lr.status)===1)?'<span class="badge bg-success-lt">Active</span>':'<span class="badge bg-danger-lt">Inactive</span>');
    if(lr.is_body_ler) h+=' <span class="badge bg-primary-lt ms-1">Body LER</span>';
    if(lr.is_global) h+=' <span class="badge bg-azure-lt ms-1">Global</span>';
    h+='</div></div>';

    h+='<div class="row g-3 mb-3">';
    h+='<div class="col-sm-6"><div class="border rounded p-2"><div class="text-muted small">Unit</div><div class="fw-medium">'+(lr.unit_name?H.esc(lr.unit_name)+(lr.unit_symbol?' ('+H.esc(lr.unit_symbol)+')':''):'<span class="text-muted">-</span>')+'</div></div></div>';
    h+='<div class="col-sm-6"><div class="border rounded p-2"><div class="text-muted small">Default Value</div><div class="fw-medium">'+(lr.default_unit_value!=null?parseFloat(lr.default_unit_value).toFixed(4):'<span class="text-muted">-</span>')+'</div></div></div>';
    h+='</div>';

    if(lr.description){
        h+='<div class="mb-3"><div class="text-muted small mb-1">Description</div><div class="border rounded p-2">'+H.esc(lr.description)+'</div></div>';
    }
    if(lr.url){
        h+='<div class="mb-3"><div class="text-muted small mb-1">URL</div><div class="border rounded p-2"><a href="'+H.esc(lr.url)+'" target="_blank" class="text-break">'+H.esc(lr.url)+' <i class="bi bi-box-arrow-up-right ms-1"></i></a></div></div>';
    }

    h+='<div class="border-top pt-3">';
    h+='<div class="mb-2"><span class="text-muted small">Created:</span> '+smsFormatDateTime(lr.created_at)+'</div>';
    h+='<div class="mb-2"><span class="text-muted small">Updated:</span> '+smsFormatDateTime(lr.updated_at)+'</div>';
    h+='</div>';
...
/* Actions */
function toggleLer(u){$.post(BASE_URL+'/lers/'+u+'/toggle-status',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}
function delLer(u,n){smsConfirm({title:'Delete LER',msg:'Are you sure you want to delete <strong>'+H.esc(n)+'</strong>?',btnClass:'btn-danger',btnText:'Delete',onConfirm:function(){showLoading();$.post(BASE_URL+'/lers/'+u+'/delete',function(r){hideLoading();if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);}).fail(function(){hideLoading();toastr.error('Network error.');});}});}
function recoverLer(u,n){smsConfirm({title:'Recover LER',msg:'Recover <strong>'+H.esc(n)+'</strong>?',btnClass:'btn-success',btnText:'Recover',onConfirm:function(){showLoading();$.post(BASE_URL+'/lers/'+u+'/recover',function(r){hideLoading();if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);}).fail(function(){hideLoading();toastr.error('Network error.');});}});}
...
function bulkAction(a){if(!_sel.length)return;smsConfirm({title:a.charAt(0).toUpperCase()+a.slice(1),msg:_sel.length+' items will be '+a+'d.',btnClass:a==='delete'?'btn-danger':'btn-primary',btnText:a.charAt(0).toUpperCase()+a.slice(1),onConfirm:function(){showLoading();$.post(BASE_URL+'/lers/bulk-action',{action:a,uuids:JSON.stringify(_sel)},function(r){hideLoading();if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);}).fail(function(){hideLoading();toastr.error('Network error.');});}});}

/* Export */
function doExport(fmt){var p=_filters();p.format=fmt;delete p.page;delete p.per_page;if(fmt==='csv'||fmt==='excel'||fmt==='pdf'){window.location.href=BASE_URL+'/lers/export?'+$.param(p);return;}showLoading();$.post(BASE_URL+'/lers/export',p,function(res){hideLoading();if(!res||res.status!==200||!res.data||!res.data.rows||!res.data.rows.length){toastr.error('No data.');return;}var rows=res.data.rows,cols=Object.keys(rows[0]);var html='<html><head><title>LER Codes</title><style>body{font-family:Arial;font-size:12px;padding:20px;}table{border-collapse:collapse;width:100%;}th,td{border:1px solid #ccc;padding:6px 8px;}th{background:#f0f4f8;font-weight:600;}tr:nth-child(even){background:#fafafa;}</style></head><body><h2>LER Codes ('+rows.length+')</h2><table><thead><tr>';cols.forEach(function(c){html+='<th>'+H.esc(c)+'</th>';});html+='</tr></thead><tbody>';rows.forEach(function(r){html+='<tr>';cols.forEach(function(c){html+='<td>'+H.esc(String(r[c]||''))+'</td>';});html+='</tr>';});html+='</tbody></table></body></html>';var w=window.open('','_blank');w.document.write(html);w.document.close();if(fmt==='print')setTimeout(function(){w.print();},400);}).fail(function(){hideLoading();toastr.error('Failed.');});}
...
function renderImportResults(results) {
    ...
    var h='<table class="table table-sm table-bordered mb-0" style="font-size:11px;white-space:nowrap;"><thead class="table-light"><tr><th style="width:28px;">#</th><th style="min-width:140px;">Name</th><th style="min-width:100px;">Code</th><th style="min-width:110px;">Error</th><th style="width:48px;">Retry</th></tr></thead><tbody>';
    results.forEach(function(r,i){if(r.status!=='error')return;var d=r.data||{};var dn=r.name||d.name||'';var dc=d.code||'';h+='<tr class="table-danger" id="impRow'+i+'"><td>'+r.row+'</td><td><input type="text" class="form-control form-control-sm imp-name" value="'+H.esc(dn)+'" placeholder="Name *"/></td><td><input type="text" class="form-control form-control-sm imp-code" value="'+H.esc(dc)+'" placeholder="Code"/></td><td class="small text-danger">'+H.esc(r.message||'Error')+'</td><td><button class="btn btn-sm btn-outline-warning" onclick="retryImportRow('+i+')"><i class="bi bi-arrow-repeat"></i></button></td></tr>';});
    h+='</tbody></table>';
