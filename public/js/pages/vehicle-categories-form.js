...
$(function() {
    ...
    function checkAIConfig() {
        $.get(BASE_URL + '/vehicle-categories/ai-config', function(res) {
    ...
    $('#btnTranslateAll').on('click', function() {
        ...
        $.ajax({
            url: BASE_URL + '/vehicle-categories/translate',
            type: 'POST',
    ...
    $(document).on('click', '.sms-img-preview', function() {
        var $img     = $(this);
        var title    = $img.data('title') || T('vehicle_categories.image_preview','Image Preview');
        ...
        if (uploaded) {
            ...
            html += '<div class="text-muted small mb-2"><i class="bi bi-cloud-upload me-1 text-success"></i><strong>' + T('vehicle_categories.image_uploaded','Uploaded') + '</strong></div>';
        ...
        $.ajax({
            ...
            success: function(r) {
                ...
                if (r.status === 200 || r.status === 201) {
                    ...
                    setTimeout(function() { window.location = '/vehicle-categories'; }, 800);
