...
$(function() {
    ...
    function checkAIConfig() {
        $.get(BASE_URL + '/part-groups/ai-config', function(res) {
    ...
    $('#btnTranslateAll').on('click', function() {
        ...
        $.ajax({
            url: BASE_URL + '/part-groups/translate',
            type: 'POST',
    ...
    $(document).on('click', '.sms-img-preview', function() {
        var $img     = $(this);
        var title    = $img.data('title') || T('part_groups.image_preview','Image Preview');
        ...
        if (uploaded) {
            ...
            html += '<div class="text-muted small mb-2"><i class="bi bi-cloud-upload me-1 text-success"></i><strong>' + T('part_groups.image_uploaded','Uploaded') + '</strong></div>';
        ...
        $.ajax({
            ...
            success: function(r) {
                ...
                if (r.status === 200 || r.status === 201) {
                    ...
                    setTimeout(function() { window.location = '/part-groups'; }, 800);
