...
function loadWarehouses(){
    ...
    $el.select2({dropdownParent:$('#filterSidebar'),placeholder:'All Warehouses',allowClear:true,width:'100%',ajax:{url:BASE_URL+'/warehouses/autocomplete',dataType:'json',delay:250,data:function(params){var d={q:params.term||''};if($('#filterCompany').length && $('#filterCompany').val() && $('#filterCompany').val()!=='all') d.company_id=$('#filterCompany').val();return d;},processResults:function(res){return{results:(res.data||[]).map(function(w){return{id:w.id,text:w.name+' ('+(w.code||'')+')'};})};}},minimumInputLength:0});
    // Company change -> clear warehouse
    $('#filterCompany').off('change.wh').on('change.wh', function(){ $el.val(null).trigger('change'); });
}

function loadZonesSelect2(warehouseId){
    var $el=$('#filterZone');
    $el.val(null).trigger('change');
    $el.select2('destroy');
    $el.select2({dropdownParent:$('#filterSidebar'),placeholder:'All Zones',allowClear:true,width:'100%',ajax:{url:BASE_URL+'/warehouse-zones/autocomplete',dataType:'json',delay:250,data:function(params){var d={q:params.term||''};if(warehouseId)d.warehouse_id=warehouseId;if($('#filterCompany').length && $('#filterCompany').val() && $('#filterCompany').val()!=='all') d.company_id=$('#filterCompany').val();return d;},processResults:function(res){return{results:(res.data||[]).map(function(z){return{id:z.id,text:z.name+' ('+(z.code||'')+')'};})};}},minimumInputLength:0});
}

function initZoneSelect2(){
    $('#filterZone').select2({dropdownParent:$('#filterSidebar'),placeholder:'All Zones',allowClear:true,width:'100%',ajax:{url:BASE_URL+'/warehouse-zones/autocomplete',dataType:'json',delay:250,data:function(params){var d={q:params.term||''};var wh=$('#filterWarehouse').val();if(wh)d.warehouse_id=wh;if($('#filterCompany').length && $('#filterCompany').val() && $('#filterCompany').val()!=='all') d.company_id=$('#filterCompany').val();return d;},processResults:function(res){return{results:(res.data||[]).map(function(z){return{id:z.id,text:z.name+' ('+(z.code||'')+')'};})};}},minimumInputLength:0});
}
...
function loadData(){
    ...
    $.post(BASE_URL+'/warehouse-shelves/paginate',_filters(),function(res){
        ...
        if(!data.length){$('#tableBody').html('<tr><td colspan="'+cols+'" class="text-center py-5 text-muted"><i class="bi bi-bookshelf d-block mb-2" style="font-size:36px;opacity:.3;"></i>No shelves found</td></tr>');$('#tableInfo').text('');$('#tablePagination').html('');return;}
        ...
        data.forEach(function(r,i){
            ...
            if(deleted){
                ...
                if(editable)acts+='<li><a class="dropdown-item" href="'+BASE_URL+'/warehouse-shelves/'+r.uuid+'/edit"><i class="bi bi-pencil me-2 text-secondary"></i>Edit</a></li>';
            ...
            var znName=(r.zone&&r.zone.name)?r.zone.name:(r.zone_name||'');
            rows+='<tr'+rowClass+'><td style="padding-left:1rem;"><input type="checkbox" class="form-check-input row-chk" data-uuid="'+r.uuid+'"/></td>'+
                ...
                '<td class="d-none d-md-table-cell">'+H.esc(znName)+'</td>'+
                '<td>'+status+'</td>'+
...
function togglePT(u){$.post(BASE_URL+'/warehouse-shelves/'+u+'/toggle-status',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}
function delPT(u,n){smsConfirm({icon:'🗑️',title:T('btn.delete','Delete'),msg:'Are you sure you want to delete <strong>'+H.esc(n)+'</strong>?',btnClass:'btn-danger',btnText:T('btn.delete','Delete'),onConfirm:function(){showLoading();$.post(BASE_URL+'/warehouse-shelves/'+u+'/delete',function(r){hideLoading();if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);}).fail(function(){hideLoading();toastr.error(T('general.network_error','Network error.'));});}});}
function recoverPT(u,n){smsConfirm({icon:'♻️',title:T('btn.recover','Recover'),msg:'Recover <strong>'+H.esc(n)+'</strong>?',btnClass:'btn-success',btnText:T('btn.recover','Recover'),onConfirm:function(){showLoading();$.post(BASE_URL+'/warehouse-shelves/'+u+'/recover',function(r){hideLoading();if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);}).fail(function(){hideLoading();toastr.error(T('general.network_error','Network error.'));});}});}
...
function showUsage(uuid, name) {
    ...
    $.get(BASE_URL + '/warehouse-shelves/' + uuid + '/usage', function(res) {
        if (!res || res.status !== 200) { $b.html('<div class="alert alert-danger m-3">'+T('general.failed_load','Failed.')+'</div>'); return; }
        smsRenderUsageBody(res.data, 'warehouse-shelves', uuid, name);
...
function viewRecord(uuid){
    ...
    $.get(BASE_URL+'/warehouse-shelves/'+uuid+'/view-data',function(res){
        if(!res||res.status!==200){$b.html('<div class="alert alert-danger m-3">'+T('general.not_found','Not found.')+'</div>');return;}
        var d=res.data&&res.data.shelf?res.data.shelf:(res.data||{});
        ...
        h+=_section('Shelf Information','bi-bookshelf',
            _r('Name',d.name)+_r('Code',d.code)+_r('Full Code',d.full_code)+_r('Full Name',d.full_name)+_r('Warehouse',d.warehouse_name)+_r('Zone',d.zone_name)+_r('Description',d.description));
...
function doExport(fmt){var p=_filters();p.format=fmt;delete p.page;delete p.per_page;if(fmt==='csv'||fmt==='excel'||fmt==='pdf'){window.location.href=BASE_URL+'/warehouse-shelves/export?'+$.param(p);return;}showLoading();$.post(BASE_URL+'/warehouse-shelves/export',p,function(res){hideLoading();if(!res||res.status!==200||!res.data||!res.data.rows||!res.data.rows.length){toastr.error(T('general.no_data','No data.'));return;}var rows=res.data.rows,cols=Object.keys(rows[0]);var html='<html><head><title>Warehouse Shelves</title><style>body{font-family:Arial;font-size:12px;padding:20px;}table{border-collapse:collapse;width:100%;}th,td{border:1px solid #ccc;padding:6px 8px;}th{background:#f0f4f8;font-weight:600;}tr:nth-child(even){background:#fafafa;}</style></head><body><h2>Warehouse Shelves ('+rows.length+')</h2><table><thead><tr>';cols.forEach(function(c){html+='<th>'+H.esc(c)+'</th>';});html+='</tr></thead><tbody>';rows.forEach(function(r){html+='<tr>';cols.forEach(function(c){html+='<td>'+H.esc(String(r[c]||''))+'</td>';});html+='</tr>';});html+='</tbody></table></body></html>';var w=window.open('','_blank');w.document.write(html);w.document.close();if(fmt==='print')setTimeout(function(){w.print();},400);}).fail(function(){hideLoading();toastr.error(T('general.failed','Failed.'));});}

$(function(){
    _pp=smsInitPerPage('#perPageSel');loadWarehouses();initZoneSelect2();loadData();
    ...
    // Cascade: warehouse change resets zone
    $('#filterWarehouse').on('change',function(){loadZonesSelect2($(this).val());});

    // Sidebar: Apply
    $('#btnApplyFilters').on('click',function(){_page=1;_updateFilterCount();loadData();});
    // Sidebar: Clear All
    $('#btnClearAdvFilters').on('click',function(){$('#filterWarehouse').val(null).trigger('change');$('#filterZone').val(null).trigger('change');$('#filterStatus,#filterDeleted').val('');if($('#filterCompany').length)$('#filterCompany').val('all');_updateFilterCount();});
    // Inline clear
    $('#btnClearFilters').on('click',function(){$('#searchInput').val('');$('#filterWarehouse').val(null).trigger('change');$('#filterZone').val(null).trigger('change');$('#filterStatus,#filterDeleted').val('');if($('#filterCompany').length)$('#filterCompany').val('all');_updateFilterCount();_page=1;loadData();});
    $('#btnClearAllFilters').on('click',function(){$('#searchInput').val('');$('#filterWarehouse').val(null).trigger('change');$('#filterZone').val(null).trigger('change');$('#filterStatus,#filterDeleted').val('');if($('#filterCompany').length)$('#filterCompany').val('all');_updateFilterCount();_page=1;loadData();});
