    ...
    $.post(BASE_URL+'/part-groups/paginate',_filters(),function(res){
        ...
        data.forEach(function(r,i){
            ...
            if(deleted){
                ...
                if(editable)acts+='<li><a class="dropdown-item" href="'+BASE_URL+'/part-groups/'+r.uuid+'/edit"><i class="bi bi-pencil me-2 text-secondary"></i>Edit</a></li>';
...
function retryImportRow(idx) {
    ...
    $.post(BASE_URL + '/part-groups/import/single', { name: nm }, function(res) {
        btnReset($b);
        if (res.status === 200 || res.status === 201) {
            $tr.removeClass('table-danger').addClass('table-success');
            $tr.find('input').prop('disabled', true).addClass('bg-light');
            $tr.find('td:eq(2)').html('<span class="text-success small"><i class="bi bi-check-circle me-1"></i>OK</span>');
