/* vehicle-variants.js — v5 full rewrite */
'use strict';
...
// Show/hide bulk recover button
    ...
    $.post(BASE_URL+'/vehicle-variants/paginate',_filters(),function(res){
        ...
        if(!data.length){$('#tableBody').html('<tr><td colspan="7" class="text-center py-5 text-muted"><i class="bi bi-gear-wide-connected d-block mb-2" style="font-size:36px;opacity:.3;"></i>'+T('vehicle_variants.no_data','No data')+'</td></tr>');$('#tableInfo').text('');$('#tablePagination').html('');return;}
        ...
        data.forEach(function(r,i){
            ...
            if(deleted){
                ...
                if(editable)acts+='<a href="'+BASE_URL+'/vehicle-variants/'+r.uuid+'/edit" class="btn btn-ghost-secondary" title="'+T('btn.edit','Edit')+'"><i class="bi bi-pencil"></i></a>';
...
function retryImportRow(idx) {
    ...
    $.post(BASE_URL + '/vehicle-variants/import/single', { name: newName, image_url: (r.data && r.data['image url']) || '' }, function(res) {
