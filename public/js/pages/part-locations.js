/* part-locations.js — v5 full rewrite */
'use strict';
...
function showBothImages($el){var n=$el.data('name')||'',ext=$el.data('ext')||'',up=$el.data('up')||'';if(!ext&&!up)return;
    $('#imgModalTitle').html('<i class="bi bi-image me-2 text-primary"></i>'+H.esc(n||T('part_locations.image_both_title','Images')));
    var h='<div class="row g-3">';
    if(up){h+='<div class="'+(ext?'col-6':'col-12')+'"><div class="border rounded p-2 text-center"><div class="text-muted small mb-2"><i class="bi bi-cloud-upload me-1 text-success"></i><strong>'+T('part_locations.image_uploaded','Uploaded')+'</strong></div><img src="'+H.esc(up)+'" class="rounded" style="max-width:100%;max-height:260px;object-fit:contain;" onerror="this.outerHTML=\'<div class=text-muted>'+T('general.no_image','No image')+'</div>\'"/><div class="mt-2"><a href="'+H.esc(up)+'" target="_blank" class="btn btn-sm btn-outline-primary" download><i class="bi bi-download me-1"></i>'+T('general.download','Download')+'</a></div></div></div>';}
...
// Show/hide bulk recover button
    ...
    $.post(BASE_URL+'/part-locations/paginate',_filters(),function(res){
        ...
        if(!data.length){$('#tableBody').html('<tr><td colspan="7" class="text-center py-5 text-muted"><i class="bi bi-gear-wide-connected d-block mb-2" style="font-size:36px;opacity:.3;"></i>'+T('part_locations.no_data','No data')+'</td></tr>');$('#tableInfo').text('');$('#tablePagination').html('');return;}
        ...
        data.forEach(function(r,i){
            ...
            if(deleted){
                ...
                if(editable)acts+='<a href="'+BASE_URL+'/part-locations/'+r.uuid+'/edit" class="btn btn-ghost-secondary" title="'+T('btn.edit','Edit')+'"><i class="bi bi-pencil"></i></a>';
...
function viewPT(uuid){var $b=$('#viewBody');$b.html('<div class="text-center py-5"><div class="spinner-border text-primary"></div></div>');bootstrap.Modal.getOrCreateInstance($('#modalView')[0]).show();
    $.get(BASE_URL+'/part-locations/'+uuid+'/view-data',function(res){
        if(!res||res.status!==200){$b.html('<div class="alert alert-danger m-3">'+T('general.not_found','Not found.')+'</div>');return;}
        var pt=res.data.part_location||res.data.part_type||{},langs=res.data.languages||[],trans=pt.translations||[];
        ...
        h+='</div>';
        if(langs.length>0){var tM={};trans.forEach(function(t){tM[t.language_id]=t.name;});h+='<div class="border-top pt-3"><h6 class="fw-semibold mb-2" style="font-size:13px;"><i class="bi bi-translate me-1 text-primary"></i>'+T('part_locations.translations','Translations')+'</h6><div class="row g-2">';langs.forEach(function(l){var v=tM[l.id]?H.esc(tM[l.id]):'<span class="text-muted fst-italic">—</span>';h+='<div class="col-sm-6"><div class="border rounded p-2"><div class="text-muted" style="font-size:11px;">'+(l.flag?l.flag+' ':'')+H.esc(l.name)+'</div><div class="fw-medium" style="font-size:13px;">'+v+'</div></div></div>';});h+='</div></div>';}
        h+='</div>';$b.html(h);}).fail(function(){$b.html('<div class="alert alert-danger m-3">'+T('general.network_error','Error.')+'</div>');});}

/* Actions */
function togglePT(u){$.post(BASE_URL+'/part-locations/'+u+'/toggle-status',function(r){if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);});}
function showUsage(uuid, name) {
    ...
    $.get(BASE_URL + '/part-locations/' + uuid + '/usage', function(res) {
        if (!res || res.status !== 200) { $b.html('<div class="alert alert-danger m-3">Failed.</div>'); return; }
        smsRenderUsageBody(res.data, 'part-locations', uuid, name);
    }).fail(function() { $b.html('<div class="alert alert-danger m-3">Network error.</div>'); });
}
function delPT(u,n){smsConfirm({icon:'🗑️',title:T('part_locations.delete','Delete'),msg:T('general.are_you_sure','Sure?')+' <strong>'+H.esc(n)+'</strong>',btnClass:'btn-danger',btnText:T('btn.delete','Delete'),onConfirm:function(){showLoading();$.post(BASE_URL+'/part-locations/'+u+'/delete',function(r){hideLoading();if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);}).fail(function(){hideLoading();toastr.error(T('general.network_error','Error.'));});}});}
function recoverPT(u,n){smsConfirm({icon:'♻️',title:T('bulk.recover','Recover'),msg:T('bulk.recover','Recover')+' <strong>'+H.esc(n)+'</strong> and its translations?',btnClass:'btn-success',btnText:T('bulk.recover','Recover'),onConfirm:function(){showLoading();$.post(BASE_URL+'/part-locations/'+u+'/recover',function(r){hideLoading();if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);}).fail(function(){hideLoading();toastr.error(T('general.network_error','Error.'));});}});}
...
function bulkAction(a){if(!_sel.length)return;smsConfirm({icon:'⚡',title:a,msg:_sel.length+' '+T('part_locations.bulk_affected','items.'),btnClass:a==='delete'?'btn-danger':'btn-primary',btnText:a,onConfirm:function(){showLoading();$.post(BASE_URL+'/part-locations/bulk-action',{action:a,uuids:JSON.stringify(_sel)},function(r){hideLoading();if(r.status===200){toastr.success(r.message);loadData();}else toastr.error(r.message);}).fail(function(){hideLoading();toastr.error(T('general.network_error','Error.'));});}});}

/* Export */
function doExport(fmt){var p=_filters();p.format=fmt;delete p.page;delete p.per_page;if(fmt==='csv'||fmt==='excel'||fmt==='pdf'){window.location.href=BASE_URL+'/part-locations/export?'+$.param(p);return;}showLoading();$.post(BASE_URL+'/part-locations/export',p,function(res){hideLoading();if(!res||res.status!==200||!res.data||!res.data.rows||!res.data.rows.length){toastr.error(T('general.no_data','No data.'));return;}var rows=res.data.rows,cols=Object.keys(rows[0]);var html='<html><head><title>Part Locations</title><style>body{font-family:Arial;font-size:12px;padding:20px;}table{border-collapse:collapse;width:100%;}th,td{border:1px solid #ccc;padding:6px 8px;}th{background:#f0f4f8;font-weight:600;}tr:nth-child(even){background:#fafafa;}</style></head><body><h2>Part Locations ('+rows.length+')</h2><table><thead><tr>';cols.forEach(function(c){html+='<th>'+H.esc(c)+'</th>';});html+='</tr></thead><tbody>';rows.forEach(function(r){html+='<tr>';cols.forEach(function(c){html+='<td>'+H.esc(String(r[c]||''))+'</td>';});html+='</tr>';});html+='</tbody></table></body></html>';var w=window.open('','_blank');w.document.write(html);w.document.close();if(fmt==='print')setTimeout(function(){w.print();},400);}).fail(function(){hideLoading();toastr.error(T('msg.failed','Failed.'));});}
/* ── Import ── */
var _pollTimer = null;
var _importResults = [];
function openImport(){
...
function pollImportStatus(jobId){
    $.get(BASE_URL+'/notifications/job/'+jobId, function(res){
        ...
        var d=res.data;
        $('#impProcessed').text((d.processed_rows||0).toLocaleString());$('#impSuccess').text((d.success_count||0).toLocaleString());$('#impErrors').text((d.error_count||0).toLocaleString());
        $('#impPercent').text((d.progress||0)+'%');$('#impProgressBar').css('width',(d.progress||0)+'%');
        if(d.status==='completed'){
            clearInterval(_pollTimer);_pollTimer=null;$('#impProgressBar').addClass(d.error_count>0?'bg-warning':'bg-success');
            if(typeof fetchUnreadCount==='function') fetchUnreadCount();
            if(d.error_count>0 && d.results && d.results.length){
                toastr.warning(T('import.done','Import done:')+' '+d.success_count+' imported, '+d.error_count+' errors.');
                setTimeout(function(){$('#importStep3').addClass('d-none');$('#importStep2').removeClass('d-none');renderImportResults(d.results);},800);
            } else {
                toastr.success('All '+d.success_count+' rows imported successfully!');
                $('#impProgressBar').addClass('bg-success').css('width','100%').text('100%');
                $('#importStep3').find('.d-flex').first().after('<div class="text-center py-3 mt-2"><div style="font-size:42px;">✅</div><h5 class="text-success mt-2">All '+d.success_count+' rows imported!</h5><p class="text-muted small">Check your notifications for details.</p></div>');
            ...
            toastr.error(T('import.failed','Import failed:')+' '+(d.error_message||'Unknown error'));if(typeof fetchUnreadCount==='function') fetchUnreadCount();
        }
...
function renderImportResults(results) {
    ...
    if (err === 0) {
        ...
        $('#importErrorActions').addClass('d-none');loadData();if(typeof fetchUnreadCount==='function') fetchUnreadCount();
        setTimeout(function(){ try{bootstrap.Modal.getOrCreateInstance($('#modalImport')[0]).hide();}catch(e){} }, 3000);return;
    }
    var h = '<table class="table table-sm table-bordered mb-0" style="font-size:11px;white-space:nowrap;"><thead class="table-light"><tr><th style="width:28px;">#</th><th style="min-width:140px;">Name</th><th style="min-width:110px;">Error</th><th style="width:48px;">Retry</th></tr></thead><tbody>';
    results.forEach(function(r, i) {
        if (r.status !== 'error') return;
        var d = r.data || {};var dn = r.name || d.name || d['name'] || '';
        h += '<tr class="table-danger" id="impRow' + i + '"><td>' + r.row + '</td><td><input type="text" class="form-control form-control-sm imp-name" value="' + H.esc(dn) + '" placeholder="Name *"/></td><td class="small text-danger">' + H.esc(r.message || 'Error') + '</td><td><button class="btn btn-sm btn-outline-warning" onclick="retryImportRow(' + i + ')"><i class="bi bi-arrow-repeat"></i></button></td></tr>';
    });
    h += '</tbody></table>';
    if (ok > 0) { $('#importResultTable').html('<div class="alert alert-success py-2 mb-3 small"><i class="bi bi-check-circle me-1"></i><strong>' + ok + ' rows imported successfully.</strong> Below are the ' + err + ' rows that need attention:</div>' + h); } else { $('#importResultTable').html(h); }
...
function retryImportRow(idx) {
    ...
    var $tr = $('#impRow' + idx);var nm = $tr.find('.imp-name').val().trim();
    if (!nm) { toastr.error(T('msg.name_required','Name is required.')); return; }
    var $b = $tr.find('button'); btnLoading($b);
    $.post(BASE_URL + '/part-locations/import/single', { name: nm }, function(res) {
        btnReset($b);
        if (res.status === 200 || res.status === 201) {
            $tr.removeClass('table-danger').addClass('table-success');$tr.find('input').prop('disabled', true).addClass('bg-light');
            $tr.find('td:eq(2)').html('<span class="text-success small"><i class="bi bi-check-circle me-1"></i>OK</span>');$tr.find('td:eq(3)').html('\u2014');
            r.status = 'success';toastr.success('"' + nm + '" imported.');loadData();
            var remaining = _importResults.filter(function(x){return x.status==='error';}).length;
            if (remaining === 0) { $('#importSummary').html('<div class="alert alert-success py-2 mb-0"><i class="bi bi-check-circle me-1"></i><strong>All rows imported successfully!</strong></div>');$('#importErrorActions').addClass('d-none');setTimeout(function(){ try{bootstrap.Modal.getOrCreateInstance($('#modalImport')[0]).hide();}catch(e){} },2000);
            } else { var ok = _importResults.filter(function(x){return x.status==='success';}).length;$('#importSummary').html('<div class="d-flex gap-2 flex-wrap"><span class="badge bg-success-lt px-3 py-2"><i class="bi bi-check-circle me-1"></i>' + ok + ' imported</span><span class="badge bg-danger-lt px-3 py-2"><i class="bi bi-x-circle me-1"></i>' + remaining + ' errors</span></div>'); }
