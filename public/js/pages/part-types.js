...
function showUsage(uuid, name) {
    ...
    $.get(BASE_URL + '/part-types/' + uuid + '/usage', function(res) {
        if (!res || res.status !== 200) { $b.html('<div class="alert alert-danger m-3">Failed.</div>'); return; }
        smsRenderUsageBody(res.data, 'part-types', uuid, name);
...
function renderImportResults(results) {
    ...
    results.forEach(function(r, i) {
        ...
        var d = r.data || {};
        var dn = r.name || r.part_name || d.name || d.part_name || d['part name'] || '';
...
function retryImportRow(idx) {
    ...
    $.post(BASE_URL + '/part-types/import/single', {
        part_name: nm, image_url: iu
    }, function(res) {
        btnReset($b);
        if (res.status === 200 || res.status === 201) {
            $tr.removeClass('table-danger').addClass('table-success');
            $tr.find('input').prop('disabled', true).addClass('bg-light');
            $tr.find('td:eq(3)').html('<span class="text-success small"><i class="bi bi-check-circle me-1"></i>OK</span>');
