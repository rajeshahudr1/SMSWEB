...
function _initSidebarSelect2(){
    ...
    $('#filterCompany').off('change.wh').on('change.wh',function(){$('#filterWarehouse').val(null).trigger('change.select2');$('#filterZone').val(null).trigger('change.select2');$('#filterShelf').val(null).trigger('change.select2');});
}
...
function loadData(){
    ...
    $.post(BASE_URL+'/warehouse-racks/paginate',_filters(),function(res){
        ...
        if(!data.length){$('#tableBody').html('<tr><td colspan="'+cols+'" class="text-center py-5 text-muted"><i class="bi bi-layout-three-columns d-block mb-2" style="font-size:36px;opacity:.3;"></i>No racks found</td></tr>');$('#tableInfo').text('');$('#tablePagination').html('');return;}
        ...
        data.forEach(function(r,i){
            ...
            if(deleted){
                ...
                if(editable)acts+='<li><a class="dropdown-item" href="'+BASE_URL+'/warehouse-racks/'+r.uuid+'/edit"><i class="bi bi-pencil me-2 text-secondary"></i>Edit</a></li>';
            ...
            var shName=(r.shelf&&r.shelf.name)?r.shelf.name:(r.shelf_name||'');
            rows+='<tr'+rowClass+'><td style="padding-left:1rem;"><input type="checkbox" class="form-check-input row-chk" data-uuid="'+r.uuid+'"/></td>'+
                ...
                '<td class="d-none d-lg-table-cell">'+H.esc(shName)+'</td>'+
                '<td>'+status+'</td>'+
...
function viewRecord(uuid){
    ...
    $.get(BASE_URL+'/warehouse-racks/'+uuid+'/view-data',function(res){
        ...
        h+=_section('Basic Information','bi-layout-three-columns',
        ...
        h+=_section('Parent Hierarchy','bi-diagram-3',
            _r('Warehouse',w.warehouse_name)+_r('Zone',w.zone_name)+_r('Shelf',w.shelf_name)+_r('Shelf Full Code',w.shelf_full_code));
...
$(function(){
    ...
    $('#filterWarehouse').on('change',function(){$('#filterZone').val(null).trigger('change.select2');$('#filterShelf').val(null).trigger('change.select2');});
    $('#filterZone').on('change',function(){$('#filterShelf').val(null).trigger('change.select2');});

    // Sidebar: Apply button
    $('#btnApplyFilters').on('click',function(){_page=1;loadData();_updateFilterBadge();});
    // Sidebar: Clear All
    $('#btnClearFilters').on('click',function(){$('#searchInput').val('');$('#filterStatus,#filterDeleted').val('');$('#filterWarehouse').val(null).trigger('change.select2');$('#filterZone').val(null).trigger('change.select2');$('#filterShelf').val(null).trigger('change.select2');if($('#filterCompany').length)$('#filterCompany').val('all');_page=1;loadData();_updateFilterBadge();});
    $('#btnClearAllFilters').on('click',function(){$('#searchInput').val('');$('#filterStatus,#filterDeleted').val('');$('#filterWarehouse').val(null).trigger('change.select2');$('#filterZone').val(null).trigger('change.select2');$('#filterShelf').val(null).trigger('change.select2');if($('#filterCompany').length)$('#filterCompany').val('all');_page=1;loadData();_updateFilterBadge();});
