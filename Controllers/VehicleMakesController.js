...
const getOrgs = (tk) => api.get('/vehicle-makes/organizations', tk).then(r => r.status === 200 ? (r.data || []) : []);
...
exports.create = async (req, res) => {
    ...
    res.render('vehicle-makes/form', {
        page_title: 'Add Vehicle Make', activeLink: 'vehicle-makes',
...
exports.edit = async (req, res) => {
    const [r, langs, orgs] = await Promise.all([
        api.get('/vehicle-makes/' + req.params.uuid, req.session.token),
        getLangs(req.session.token),
        req.session.user && req.session.user.is_super_admin ? getOrgs(req.session.token) : Promise.resolve([]),
    ]);
    if (r.status !== 200) { req.flash('error', 'Not found.'); return res.redirect('/vehicle-makes'); }
    res.render('vehicle-makes/form', {
        page_title: 'Edit Vehicle Make', activeLink: 'vehicle-makes',
...
exports.store = [(r,s,n) => { tempUpload.single('image')(r,s,(e) => { if (e) return s.json({ status: 422, message: e.message }); n(); }); }, async (req, res) => {
    try { const fd = new FormData(); Object.keys(req.body).forEach(k => { if (req.body[k] != null) fd.append(k, String(req.body[k])); }); if (req.file) fd.append('image', fs.createReadStream(req.file.path), { filename: req.file.originalname, contentType: req.file.mimetype }); const r = await api.postForm('/vehicle-makes', fd, req.session.token); clean(req.file); return res.json({ status: r.status, message: r.message, data: r.data || null }); } catch(e) { clean(req.file); return res.json({ status: 500, message: 'Error.' }); }
}];
...
exports.update = [(r,s,n) => { tempUpload.single('image')(r,s,(e) => { if (e) return s.json({ status: 422, message: e.message }); n(); }); }, async (req, res) => {
    ...
    const axios = require('axios'); const BASE = process.env.API_URL || 'http://localhost:3000/api'; const h = { ...fd.getHeaders(), Authorization: 'Bearer ' + req.session.token }; const r = await axios.put(BASE + '/vehicle-makes/' + req.params.uuid, fd, { headers: h }); clean(req.file); return res.json(r.data); } catch(e) { clean(req.file); return res.json(e.response && e.response.data ? e.response.data : { status: 500, message: 'Failed.' }); }
}];

exports.viewData = async (req, res) => { res.json(await api.get('/vehicle-makes/' + req.params.uuid + '/view', req.session.token)); };
exports.destroy = async (req, res) => { res.json(await api.del('/vehicle-makes/' + req.params.uuid, req.session.token)); };
exports.toggleStatus = async (req, res) => { res.json(await api.patch('/vehicle-makes/' + req.params.uuid + '/toggle-status', {}, req.session.token)); };
exports.recover = async (req, res) => { res.json(await api.post('/vehicle-makes/' + req.params.uuid + '/recover', {}, req.session.token)); };
exports.usage = async (req, res) => { res.json(await api.get('/vehicle-makes/' + req.params.uuid + '/usage', req.session.token)); };
exports.bulkAction = async (req, res) => { res.json(await api.post('/vehicle-makes/bulk-action', req.body, req.session.token)); };

// Import single error row retry
exports.importSingleRow = async (req, res) => {
    const result = await api.post('/vehicle-makes/import/single', req.body, req.session.token);
    res.json(result);
};

exports.exportData = async (req, res) => {
    const params = Object.assign({}, req.query, req.body); const format = params.format || 'csv';
    const result = await api.post('/vehicle-makes/export/data', params, req.session.token);
...
exports.importData = [(r,s,n) => { tempImport.single('file')(r,s,(e) => { if (e) return s.json({ status: 422, message: e.message }); n(); }); }, async (req, res) => {
    try { if (!req.file) return res.json({ status: 422, message: 'No file.' }); const fd = new FormData(); fd.append('file', fs.createReadStream(req.file.path), { filename: req.file.originalname, contentType: req.file.mimetype }); const r = await api.postForm('/vehicle-makes/import/upload', fd, req.session.token); clean(req.file); return res.json(r); } catch(e) { clean(req.file); return res.json({ status: 500, message: 'Failed.' }); }
}];

// AI Translation
exports.aiConfig = async (req, res) => {
    const result = await api.get('/vehicle-makes/ai-config', req.session.token);
    res.json(result);
};

exports.translate = async (req, res) => {
    const result = await api.post('/vehicle-makes/translate', req.body, req.session.token);
    res.json(result);
};
