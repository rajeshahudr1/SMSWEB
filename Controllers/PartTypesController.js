...
const getOrgs = (tk) => api.get('/part-types/organizations', tk).then(r => r.status === 200 ? (r.data || []) : []);

exports.index = async (req, res) => {
    let orgs = [];
    if (req.session.user && req.session.user.is_super_admin) orgs = await getOrgs(req.session.token);
    res.render('part-types/index', { page_title: res.locals.t ? res.locals.t('part_types.title') : 'Part Types', activeLink: 'part-types', breadcrumbs: [{ name: 'Dashboard', url: '/dashboard' }, { name: 'Part Types', url: '/part-types' }], organizations: orgs });
};

exports.paginate = async (req, res) => { res.json(await api.post('/part-types/paginate', req.body, req.session.token)); };
...
exports.create = async (req, res) => {
    ...
    res.render('part-types/form', {
        page_title: 'Add Part Type', activeLink: 'part-types',
        breadcrumbs: [{ name: 'Dashboard', url: '/dashboard' }, { name: 'Part Types', url: '/part-types' }, { name: 'Add', url: '' }],
...
exports.edit = async (req, res) => {
    ...
    if (r.status !== 200) { req.flash('error', 'Not found.'); return res.redirect('/part-types'); }
    res.render('part-types/form', {
        page_title: 'Edit Part Type', activeLink: 'part-types',
        breadcrumbs: [{ name: 'Dashboard', url: '/dashboard' }, { name: 'Part Types', url: '/part-types' }, { name: 'Edit', url: '' }],
...
exports.store = [(r,s,n) => { tempUpload.single('image')(r,s,(e) => { if (e) return s.json({ status: 422, message: e.message }); n(); }); }, async (req, res) => {
    try { const fd = new FormData(); Object.keys(req.body).forEach(k => { if (req.body[k] != null) fd.append(k, String(req.body[k])); }); if (req.file) fd.append('image', fs.createReadStream(req.file.path), { filename: req.file.originalname, contentType: req.file.mimetype }); const r = await api.postForm('/part-types', fd, req.session.token); clean(req.file); return res.json({ status: r.status, message: r.message, data: r.data || null }); } catch(e) { clean(req.file); return res.json({ status: 500, message: 'Error.' }); }
}];
...
exports.update = [(r,s,n) => { tempUpload.single('image')(r,s,(e) => { if (e) return s.json({ status: 422, message: e.message }); n(); }); }, async (req, res) => {
    ...
    const axios = require('axios'); const BASE = process.env.API_URL || 'http://localhost:3000/api'; const h = { ...fd.getHeaders(), Authorization: 'Bearer ' + req.session.token }; const r = await axios.put(BASE + '/part-types/' + req.params.uuid, fd, { headers: h }); clean(req.file); return res.json(r.data); } catch(e) { clean(req.file); return res.json(e.response && e.response.data ? e.response.data : { status: 500, message: 'Failed.' }); }
}];

exports.usage = async (req, res) => { res.json(await api.get('/part-types/' + req.params.uuid + '/usage', req.session.token)); };
exports.viewData = async (req, res) => { res.json(await api.get('/part-types/' + req.params.uuid + '/view', req.session.token)); };
exports.destroy = async (req, res) => { res.json(await api.del('/part-types/' + req.params.uuid, req.session.token)); };
exports.toggleStatus = async (req, res) => { res.json(await api.patch('/part-types/' + req.params.uuid + '/toggle-status', {}, req.session.token)); };
exports.recover = async (req, res) => { res.json(await api.post('/part-types/' + req.params.uuid + '/recover', {}, req.session.token)); };
exports.bulkAction = async (req, res) => { res.json(await api.post('/part-types/bulk-action', req.body, req.session.token)); };

// Import single error row retry
exports.importSingleRow = async (req, res) => {
    const result = await api.post('/part-types/import/single', req.body, req.session.token);
    res.json(result);
};
...
exports.exportData = async (req, res) => {
    ...
    if (format === 'csv') { const hd = Object.keys(rows[0]); const csv = [hd.join(','), ...rows.map(r => hd.map(h => `"${(r[h]||'').toString().replace(/"/g,'""')}"`).join(','))].join('\n'); res.setHeader('Content-Type', 'text/csv'); res.setHeader('Content-Disposition', `attachment; filename="part-types-${Date.now()}.csv"`); return res.send(csv); }
    if (format === 'excel') { try { const X = require('xlsx'); const ws = X.utils.json_to_sheet(rows); const wb = X.utils.book_new(); X.utils.book_append_sheet(wb, ws, 'Part Types'); const buf = X.write(wb, { type: 'buffer', bookType: 'xlsx' }); res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'); res.setHeader('Content-Disposition', `attachment; filename="part-types-${Date.now()}.xlsx"`); return res.send(buf); } catch(e) { return res.json({ status: 200, data: result.data }); } }
    if (format === 'pdf') { const pdfExport = require('../helpers/pdfExport'); return pdfExport.generate(res, 'Part Types', rows); }
